...
<h4>思路</h4>
<p>​	字符串构造问题，按照给定规则构造字符串。需要注意的是，当连续的两个字母均为辅音的时候，中间必然存在'.'，所以我们只需要先判断两辅音连着的情况。然后将该问题排除后，我们只需要解决元辅音交替的情况下，该怎么处理。对于该问题，我们只需要在每两个辅元后面加一个'.'即可，且特判连续的最后一个位置，防止将最后一组排除。</p>
<h4>代码实现</h4>
<pre><code class="language-c++">char sum[240000];
void solve() {
    ll n;cin&gt;&gt;n;string k;cin&gt;&gt;k;
    map&lt;char,ll&gt;p;
    p['b']=p['c']=p['d']=1;
    ll id=0;
    for(int i=0;i&lt;n;i++){
        sum[id++]=k[i];
        if(i+1&lt;n&amp;&amp;p[k[i]]==p[k[i+1]]&amp;&amp;p[k[i]]==1){
            sum[id++]='.';
        }  
    }
    ll cnt=0;
